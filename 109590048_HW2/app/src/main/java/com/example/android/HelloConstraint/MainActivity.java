...
public class MainActivity extends AppCompatActivity {
    ...
    */
    public void countUp(View view) {
        mCount++;
        zerobtn.setBackgroundColor(0xFF30A6F8);
        if (mShowCount != null)
            mShowCount.setText(Integer.toString(mCount));
        if (mCount % 2 == 0)
            countbtn.setBackgroundColor(0xFFFF0000);
        else
            countbtn.setBackgroundColor(0xFF00FF00);
    }
    public void ClearZero(View view) {
        zerobtn.setBackgroundColor(0xFF9C9DA3);
        if(mCount != 0)
            mCount = 0;
            countbtn.setBackgroundColor(0xFFFF0000);
        if (mShowCount != null)
